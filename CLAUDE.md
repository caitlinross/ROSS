...
Rensselaer's Optimistic Simulation System — a parallel discrete-event simulator (PDES) written in C, using MPI for distribution. Models are collections of Logical Processes (LPs) that exchange timestamped events. The Time Warp mechanism allows optimistic execution with rollback; ROSS implements this via **reverse computation** (each event handler has a paired reverse handler that undoes its effect) rather than state saving. Downstream consumers include [CODES](../codes) (which links ROSS via pkg-config) and RISA (in-situ analysis submodule).

## Build

Presets (Ninja) are the canonical flow:

```
cmake --preset ross-debug      # configures to build/debug, installs to install/debug
cmake --build --preset ross-debug
cmake --install build/debug
```

There's also `ross-release`. Both set `BUILD_TESTING=ON`. Models are **not** built by default — pass `-DROSS_BUILD_MODELS=ON` to include `phold` and any symlinked-in models.

Without presets, a typical manual configure:
```
...
```
ctest --test-dir build/debug          # full suite
ctest --test-dir build/debug -R phold_SCHED_Optimistic   # single test by regex
ctest --test-dir build/debug -V -R <name>                # with output
```
...
### Pluggable components (selected at CMake configure time)
...
## Build-system modernization (in-flight)

Branch `cmake-improvements` has [plan.md](plan.md) and [plan-checklist.md](plan-checklist.md) describing a multi-phase CMake rewrite: add a proper `find_package(ross CONFIG)` export with `ross::ross` namespaced target, stop leaking absolute MPI paths into the exported interface, move headers under `include/ross/`, replace the model `FILE(GLOB_RECURSE)` with explicit `add_subdirectory`, etc. Phase 1 is the critical one for unblocking CODES. If a CMake change is requested, check the plan first — the intended direction is already documented there.

## Consumer compatibility (CODES)

CODES discovers ROSS via `pkg_check_modules(ROSS REQUIRED IMPORTED_TARGET ross)` using `ross.pc`. It relies on the `ROSS_INCLUDE_DIRS` variable and links `PkgConfig::ROSS`. Any install-layout change (header paths, pkg-config contents) must preserve that surface or CODES breaks. See plan.md Part 2 for the full contract.
